...
void Bitmap64AofRewrite(RedisModuleIO* aof, RedisModuleString* key, void* value) {
  ...
  uint64_t *values = RedisModule_Alloc(sizeof(uint64_t) * cardinality);
  if (values == NULL) {
      return;
  }
...
int R64SetIntArrayCommand(RedisModuleCtx* ctx, RedisModuleString** argv, int argc) {
  ...
  uint64_t* values = RedisModule_Alloc(sizeof(*values) * length);
...
int R64RangeIntArrayCommand(RedisModuleCtx* ctx, RedisModuleString** argv, int argc) {
  ...
  size_t n = 0;
  if (bitmap != NULL) {
    long long count = (long long) bitmap64_get_cardinality(bitmap);
...
int R64AppendIntArrayCommand(RedisModuleCtx* ctx, RedisModuleString** argv, int argc) {
  ...
  if (type == REDISMODULE_KEYTYPE_EMPTY) {
    ...
    Bitmap64** bitmaps = RedisModule_Alloc(2 * sizeof(*bitmaps));
    bitmaps[0] = bitmap;
    bitmaps[1] = nbitmap;
...
int R64BitOp(RedisModuleCtx* ctx, RedisModuleString** argv, int argc, Bitmap64* (* operation)(uint32_t, const Bitmap64**)) {
  ...
  Bitmap64** bitmaps = RedisModule_Alloc(n * sizeof(*bitmaps));
...
int R64BitPosCommand(RedisModuleCtx* ctx, RedisModuleString** argv, int argc) {
  ...
  if (type == REDISMODULE_KEYTYPE_EMPTY) {
    ...
    Bitmap64* bitmap = RedisModule_ModuleTypeGetValue(key);
    if (bit == 1) {
      pos = bitmap64_get_nth_element_present(bitmap, 1);
    } else {
      pos = bitmap64_get_nth_element_not_present(bitmap, 1);
    }
  }
...
int RedisModule_OnLoad(RedisModuleCtx* ctx) {
  ...
  if (RedisModule_CreateCommand(ctx, "R64.RANGEINTARRAY", R64RangeIntArrayCommand, "readonly", 1, 1, 1) ==
      REDISMODULE_ERR) {
    return REDISMODULE_ERR;
  }
