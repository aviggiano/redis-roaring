...
void test_bitmap64_andor() {
  ...
  DESCRIBE("bitmap64_andor")
  {
    ...
    {
      // bitmap1: empty
      Bitmap64* bitmap1 = roaring64_bitmap_create();
    ...
    IT("Should handle large number of bitmaps")
    {
      ...
      Bitmap64* bitmaps_array[num_bitmaps];
      const Bitmap64* bitmaps_ptrs[num_bitmaps];
      ...
      for (size_t i = 1; i < num_bitmaps; i++) {
        bitmaps_array[i] = roaring64_bitmap_create();
        roaring64_bitmap_add(bitmaps_array[i], i + 1); // Each bitmap has unique value
        roaring64_bitmap_add(bitmaps_array[i], 3);     // All have value 3
        bitmaps_ptrs[i] = bitmaps_array[i];
      }

      Bitmap64* result = roaring64_bitmap_create();
      bitmap64_andor(result, num_bitmaps, bitmaps_ptrs);
      ...
      ASSERT_TRUE(roaring64_bitmap_contains(result, 3));

      // Clean up
      for (size_t i = 0; i < num_bitmaps; i++) {
        roaring64_bitmap_free(bitmaps_array[i]);
      }
      roaring64_bitmap_free(result);
    }
    ...
    IT("Should not modify input bitmaps")
    {
      ...
      roaring64_bitmap_free(bitmap2);
      roaring64_bitmap_free(bitmap2_copy);
      roaring64_bitmap_free(result);
    }
  }
}
