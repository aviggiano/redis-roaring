...
`tests/fuzz/fuzz_manifest.json` is the authoritative command-surface coverage contract for fuzzing. Every key-bearing command must appear there, the live metadata-fuzzer table (`FUZZ_METADATA_SPECS` in `tests/fuzz/fuzz_command_metadata.c`) must enumerate the same command set, and every manifest target must have:
...
`python3 ./scripts/validate_fuzz_manifest.py` is the CI gate and runs on every push and pull request. `./scripts/check_fuzz_coverage.sh` is the broader local consistency check that also compares command registration, live metadata-fuzzer coverage, and the CMake/workflow fuzzer inventories.
...
3. Update `FUZZ_METADATA_SPECS` in `tests/fuzz/fuzz_command_metadata.c` so the metadata fuzzer keeps covering the full manifest.
